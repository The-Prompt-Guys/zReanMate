...
export const PracticeSetupPage = () => {
  ...
  const mockExam = params.get('mock') === '1';
  ...
  const start = async () => {
    if (mockExam) {
      ...
      navigate(`/practice/session?sessionId=${data.session.id}`);
      return;
    }

    window.sessionStorage.setItem('reanmate:practice-setup', JSON.stringify({ studyKitId: kit.id, ...(sourceId && { sourceId }), mode: 'practice', questionCount: count, answerFormat: format, timerSeconds: timer }));
    navigate(sourceId ? `/practice/lessons?kitId=${encodeURIComponent(kit.id)}&sourceId=${encodeURIComponent(sourceId)}` : '/practice/lessons');
  };
